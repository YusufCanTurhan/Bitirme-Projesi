...
function Planla() {
  ...
  // Sürükleme işlemi bittiğinde çalışan fonksiyon
        ...
        <DragDropContext onDragEnd={onDragEnd}>
          <div className="flex-1 flex flex-col min-h-0 px-8 pb-8 overflow-hidden">
            ...
            <div className="flex gap-3 mb-6 shrink-0">
              <button className="bg-indigo-600 text-white px-6 py-2.5 rounded-2xl text-sm font-bold shadow-lg shadow-indigo-200">1. Gün</button>
              <button className="bg-slate-50 text-slate-500 hover:bg-slate-100 px-6 py-2.5 rounded-2xl text-sm font-bold transition-all border border-slate-100">Gelecek Günler</button>
            </div>

            {/* Dikey Kaydırılabilir İçerik Alanı */}
            <div className="flex-1 overflow-y-auto pr-2 scrollbar-hide">
              
              {/* 1. GÜN PLANI (Asıl Rota) */}
              ...
              <Droppable droppableId="day1">
                {(provided, snapshot) => (
                  ...
                  >
                    ...
                    {columns.day1.items.map((item, index) => (
                        ...
                        <Draggable key={item.id} draggableId={item.id} index={index}>
                          {(provided, snapshot) => (
                            <div
                              ...
                              {...provided.draggableProps}
                              {...provided.dragHandleProps}
                              className={`bg-white border border-slate-100 rounded-2xl p-3 shadow-sm flex items-center gap-3 transition-all ${snapshot.isDragging ? 'shadow-2xl ring-2 ring-indigo-500 z-[2000]' : 'hover:border-slate-200'}`}
                            >
